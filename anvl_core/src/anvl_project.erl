...
read_patch(pcfg, Model) ->
  {ok, ConfTree} = get(?mt_conf_tree_key),
  {ok, Overrides} = get(?mt_conf_overrides),
...
?MEMO(config_loaded, Dir,
      begin
        ...
        {T, _} = timer:tc(fun() -> load_project_conf(IsNew, Dir, Project, Conf) end),
        ?LOG_WARNING("L ~s ~p", [Dir, T / 1000]),
        false
      end).
